...
template <class TileShape_MK_, int kLogMaxSplits_, int kNThreads, int AlignmentLSE_, bool Is_even_K,
          bool Varlen, class Element, class ElementPartial, class ArchTag_>
...
public:
  ...
  using GmemCopyAtom = std::conditional_t<
      Has_cp_async, cute::Copy_Atom<SM80_CP_ASYNC_CACHEGLOBAL<uint128_t>, ElementPartial>,
      cute::Copy_Atom<AutoVectorizingCopyWithAssumedAlignment<128>, ElementPartial>>;
  using GmemLayoutAtom =
      Layout<Shape<Int<MaxThreadsPerBlock / kGmemThreadsPerRow>, Int<kGmemThreadsPerRow>>,
  ...
  using GmemTiledCopy = decltype(make_tiled_copy(
      Copy_Atom<AutoVectorizingCopyWithAssumedAlignment<128>, Element>{}, GmemLayoutAtom{},
      Layout<Shape<_1, Int<kGmemElemsPerLoad>>>{})); // Val layout, 4 vals per load
  ...
  static constexpr int kBlockMSmem =
      kBlockM % 128 == 0
          ? 128
          : (kBlockM % 64 == 0 ? 64 : (kBlockM % 32 == 0 ? 32 : (kBlockM % 16 == 0 ? 16 : 8)));
  ...
  using GmemLayoutAtomLSE =
      Layout<Shape<Int<MaxThreadsPerBlock / kGmemThreadsPerRowLSE>, Int<kGmemThreadsPerRowLSE>>,
  ...
  using GmemCopyAtomLSE = std::conditional_t<
      Has_cp_async, cute::Copy_Atom<SM80_CP_ASYNC_CACHEALWAYS<AlignmentTypeLSE>, float>,
      cute::Copy_Atom<AutoVectorizingCopyWithAssumedAlignment<AlignmentLSE * sizeof(float) * 8>,
                      float>>;
  ...
  using SmemLayoutAtomLSE = decltype(composition(
      SmemLSESwizzle{}, Layout<Shape<Int<8>, Int<kBlockMSmem>>, Stride<Int<kBlockMSmem>, _1>>{}));
  using SmemLayoutLSE =
      decltype(tile_to_shape(SmemLayoutAtomLSE{}, Shape<Int<kMaxSplits>, Int<kBlockM>>{}));
  ...
  // E.g., if kBlockM is divisible by 64, and there are 256 threads, we want 4 threads (0, 1, 2, 4)
  // per column have have 64 such quads.
  ...
  using S2RLayoutAtomLSE =
      Layout<Shape<Int<kSmemThreadsPerColLSEt>, Int<MaxThreadsPerBlock / kSmemThreadsPerColLSEt>>>;
  using S2RTiledCopyLSE = decltype(make_tiled_copy(cute::Copy_Atom<cute::DefaultCopy, float>{},
                                                   S2RLayoutAtomLSE{}, Layout<_1>{}));

  using ShapeOPartial = cute::Shape<int32_t, int32_t, int32_t, int32_t,
  ...
  using ShapeLSEPartial =
      cute::Shape<int32_t, int32_t, int32_t, int32_t>; // (seqlen, num_splits, head, batch)
  using StrideLSEPartial =
      cute::Stride<_1, int64_t, int64_t, int64_t>; // (seqlen, num_splits, head, batch)
  ...
  void operator()(Params const &params, char *smem_buf) {
    ...
    Tensor sLSE =
        make_tensor(make_smem_ptr(shared_storage.smem_lse_partial.data()), SmemLayoutLSE{});
    Tensor sMaxValidSplit = make_tensor(make_smem_ptr(shared_storage.smem_max_valid_split.data()),
                                        Shape<Int<kBlockM>>{});
    ...
    int const batch = params.varlen_batch_idx_ptr ? params.varlen_batch_idx_ptr[maybe_virtual_batch]
                                                  : maybe_virtual_batch;
    ...
    if (params.semaphore_to_reset && threadIdx.x == 0 && blockIdx.x == gridDim.x - 1 &&
        blockIdx.y == gridDim.y - 1 && blockIdx.z == gridDim.z - 1) {
    ...
    flash::SeqlenInfo<Varlen, kBlockM> seqlen_info{batch, size<0>(params.shape_LSE_partial),
    ...
    Tensor mLSEpartial = make_tensor(
        make_gmem_ptr(params.ptr_LSE_partial + offset * get<0>(params.stride_LSE_partial)),
    ...
    Tensor cLSE = make_identity_tensor(
        make_shape(size<0>(sLSE), size<1>(sLSE))); // (NUM_SPLITS, BLK_M) -> (num_splits, blk_m)
...
#pragma unroll
        for (int s = 0; s < size<1>(tLSEcLSE); ++s) {
          ...
          // if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && thread_idx < 32) {
          // printf("thread_idx = %d, m = %d, s = %d, addr = %p, bank = %d\n", thread_idx, m, s,
          // reinterpret_cast<float *>(&(tLSEsLSE(_0{}, s, m))),
          // reinterpret_cast<int>(&(tLSEsLSE(_0{}, s, m))) / 4 % 32);}
          if (si < num_splits) {
...
#pragma unroll
      for (int m = 0; m < size<1>(tOcO); ++m) {
        if (tObidh(m) >= 0) {
          Tensor mOpartial_cur =
              make_tensor(make_gmem_ptr(tOrOptr[m]), mOpartial(_0{}, _, _, _0{}).layout());
          Tensor mOpartial_cur_copy =
              cute::tiled_divide(mOpartial_cur, Shape<Int<kGmemElemsPerLoad>>{});
#pragma unroll
          for (int k = 0; k < size<2>(tOcO); ++k) {
            int k_idx = get<1>(tOcO(_0{}, _0{}, k)) / kGmemElemsPerLoad;
            if (Is_even_K || tOpO(k)) {
              cute::copy(gmem_tiled_copy_O_partial, mOpartial_cur_copy(_, k_idx, split),
...
#pragma unroll
      for (int s = 0; s < size<1>(ts2rrLSE); ++s) {
        ...
        // if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && thread_idx < 32) {
        // printf("thread_idx = %d, m = %d, s = %d, addr = %p, bank = %d\n", thread_idx, m, s,
        // reinterpret_cast<float *>(&(ts2rsLSE(_0{}, s, m))),
      ...
      for (int m = 0; m < size<2>(ts2rrLSE); ++m) {
        if (get<0>(ts2rcLSE(_0{}, _0{}, m)) ==
          ...
          if (idx < max_idx) {
            ...
            // printf("thread_idx = %d, m = %d, mi = %d, idx = %d, m_idx = %d, bidh = %d, bidb = %d,
...
#pragma unroll
    ...
    Tensor mO = make_tensor(make_gmem_ptr(params.ptr_O + offset * get<0>(params.stride_O) +
