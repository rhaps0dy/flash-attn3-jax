...
template <int Arch, int kBlockM, int kBlockK, int kLogMaxSplits, bool IsEvenK, bool Varlen,
          typename Element, typename ElementPartial>
void run_flash_fwd_combine(Flash_fwd_params &params, cudaStream_t stream, bool enable_pdl) {
  ...
  using CombineKernel =
      flash::FlashAttnFwdCombine<TileShape_MK, kLogMaxSplits, 256 /*kNThreads*/, 1 /*AlignmentLSE*/,
  ...
  typename CombineKernel::Arguments args{
      ...
      {!Varlen ? params.seqlen_q : params.total_q, params.dv, params.num_splits, params.h,
       !Varlen ? params.b : 1}, // shape_O_partial
      {params.oaccum_row_stride, _1{}, params.oaccum_split_stride, params.oaccum_head_stride,
  ...
  cutlass::kernel_launch<CombineKernel>(grid_m, CombineKernel::MaxThreadsPerBlock, smem_size,
                                        stream, kernel_params,
                                        Arch >= 90 && enable_pdl /*launch_with_pdl*/);
...
void run_mha_fwd_combine_(Flash_fwd_params &params, cudaStream_t stream, bool enable_pdl) {
  ...
  // E.g., if hdim is 64, we want kBlockM to be 16 so that we can use 256 threads, each reading 4
  // elements (floats).
    ...
    BOOL_SWITCH(params.cu_seqlens_q || params.seqused_q, Varlen, [&] {
      if constexpr (kBlockM >= 16) { // If kBlockM == 8 then the minimum number of splits is 32.
        if (params.num_splits <= 16) {
          run_flash_fwd_combine<Arch, kBlockM, kBlockK, 4, false /*IsEvenK*/, Varlen, T, Tpartial>(
              params, stream, enable_pdl);
          return;
        }
      }
      if (params.num_splits <= 32) {
        run_flash_fwd_combine<Arch, kBlockM, kBlockK, 5, false /*IsEvenK*/, Varlen, T, Tpartial>(
            params, stream, enable_pdl);
      } else if (params.num_splits <= 64) {
        run_flash_fwd_combine<Arch, kBlockM, kBlockK, 6, false /*IsEvenK*/, Varlen, T, Tpartial>(
            params, stream, enable_pdl);
      } else if (params.num_splits <= 128) {
        run_flash_fwd_combine<Arch, kBlockM, kBlockK, 7, false /*IsEvenK*/, Varlen, T, Tpartial>(
            params, stream, enable_pdl);
      } else {
        run_flash_fwd_combine<Arch, kBlockM, kBlockK, 8, false /*IsEvenK*/, Varlen, T, Tpartial>(
            params, stream, enable_pdl);
      }
    });
  });
