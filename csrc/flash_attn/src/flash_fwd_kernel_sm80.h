...
public:
  ...
  // overlap with smem_v + smem_k and not smem_q and nothing else, so we'll pad in case
  // sizeof(smem_o) > sizeof(smem_v) + sizeof(smem_k).
  ...
  static dim3 get_grid_shape(Params const &params) {
    return TileScheduler::get_grid_shape(params.scheduler,
                                         params.hw_info.sm_count * MinBlocksPerMultiprocessor);
  }
  ...
  void operator()(Params const &params, char *smem_buf) {
    ...
    for (auto work_tile_info =
      ...
      if constexpr (Is_FP8 && !Has_softcap) {
        ...
        float const q_descale =
            params.mainloop.ptr_q_descale == nullptr
                ? 1.0f
                : params.mainloop.ptr_q_descale[bidb * get<0>(params.mainloop.stride_q_descale) +
                                                bidh_kv * get<1>(params.mainloop.stride_q_descale)];
        float const k_descale =
            params.mainloop.ptr_k_descale == nullptr
                ? 1.0f
                : params.mainloop.ptr_k_descale[bidb * get<0>(params.mainloop.stride_k_descale) +
                                                bidh_kv * get<1>(params.mainloop.stride_k_descale)];
        softmax_scale_log2 *= q_descale * k_descale;
      }
      flash::Softmax<2 * (2 * kBlockM / NumThreads), /*Max_offset=*/!Is_FP8 ? 0 : 8> softmax(
          softmax_scale_log2);
      ...
      if constexpr (AppendKV) {
        bool tile_new_valid = mainloop.store_kv_new(params.mainloop, threadIdx.x, shared_storage,
                                                    seqlen_info, block_coord);
        if (tile_new_valid) {
          __syncthreads();
        }
      }
      bool tile_valid = mainloop.mma(params.mainloop, tOrO, softmax, threadIdx.x, seqlen_info,
                                     block_coord, shared_storage);
      scheduler.prefetch_next_work(params.scheduler, work_tile_info);
      if (tile_valid) {
        // if (threadIdx.x == 128) { printf("Before epilogue, bid.x = %d, bid.y = %d, bid.z = %d,
        // m_block = %d, bidb = %d, split_idx = %d\n", blockIdx.x, blockIdx.y, blockIdx.z, m_block,
        // bidb, split_idx); }
