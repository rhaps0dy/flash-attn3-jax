...
public:
  ...
  using BarrierQ = std::conditional_t<Use_TMA_Q, cutlass::arch::ClusterTransactionBarrier,
  ...
  // We overlap the shared memory for the mainloop and epilogue. However, we only want smem_o to
  // overlap with smem_v and nothing else, so we'll pad in case sizeof(smem_o) > sizeof(smem_v).
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    if (sm_count <= 0) {
      CUTLASS_TRACE_HOST("  WARNING: Arguments do not include a valid SM count.\n"
                         "  For optimal performance, populate the arguments KernelHardwareInfo "
                         "struct with the SM count.");
      sm_count =
  ...
  void operator()(Params const &params, char *smem_buf) {
    ...
    // If we don't need to transpose V, we use pipeline_v for the TMA, and pipeline_vt won't be
    // used. Technically for pipeline_params_vt, warp0 of WG0 is the producer and all of WG0 are
    // consumers. However, the thread role isn't used in the pipeline implementation.
    ...
    auto pipeline_k_new = cute::conditional_return<AppendKV>(
        MainloopPipelineKVNew(shared_storage.pipelines.pipeline_k_new, pipeline_params_kv_new,
                              ClusterShape{}),
        nullptr);
    ...
    auto pipeline_v_new = cute::conditional_return<AppendKV>(
        MainloopPipelineKVNew(shared_storage.pipelines.pipeline_v_new, pipeline_params_kv_new,
                              ClusterShape{}),
        nullptr);
    ...
    TileScheduler scheduler(reinterpret_cast<typename TileScheduler::SharedStorage *>(
    ...
    if (warp_group_idx == 0) { // Producer
      ...
      for (auto work_tile_info =
        ...
        SeqlenInfo_t seqlen_info{get<2>(block_coord) /*bidb*/,
        ...
        if constexpr (AppendKV) {
          bool tile_new_valid = mainloop.load_kv_new(
              params.mainloop, pipeline_k_new, pipeline_v_new, smem_pipe_write_new, shared_storage,
      ...
      mainloop.load_tail(pipeline_k, pipeline_v, pipeline_vt, smem_pipe_write, shared_storage,
                         work_idx);
    } else { // Consumer
      ...
      for (auto work_tile_info =
        ...
        if constexpr (AppendKV) {
          ...
          if (tile_new_valid) {
            ...
            // We need this sync so that the gmem write from the consumers is visible to the
            // producer that might do TMA read after that.
        ...
        if constexpr (Is_FP8 && !Has_softcap) {
          ...
          float const q_descale =
              params.mainloop.ptr_q_descale == nullptr
                  ? 1.0f
                  : params.mainloop
                        .ptr_q_descale[bidb * get<0>(params.mainloop.stride_q_descale) +
                                       bidh_kv * get<1>(params.mainloop.stride_q_descale)];
          float const k_descale =
              params.mainloop.ptr_k_descale == nullptr
                  ? 1.0f
                  : params.mainloop
                        .ptr_k_descale[bidb * get<0>(params.mainloop.stride_k_descale) +
        ...
        bool tile_valid;
        if constexpr (!LargeHeadDimV) {
          tile_valid = mainloop.mma(params.mainloop, pipeline_k, pipeline_v, smem_pipe_read, tOrO,
                                    softmax, threadIdx.x - MmaThreadOffset, work_idx, seqlen_info,
          ...
          if (warp_group_idx == 1) {
            ...
            tile_valid = mainloop.mma_pv(params.mainloop, pipeline_v, smem_pipe_read, tOrO, softmax,
                                         threadIdx.x - MmaThreadOffset, seqlen_info, block_coord,
                                         shared_storage);
          }
        }
        ...
        if (tile_valid) {
          // if (threadIdx.x == 128) { printf("Before epilogue, bid.x = %d, bid.y = %d, bid.z = %d,
          // m_block = %d, bidb = %d, split_idx = %d\n", blockIdx.x, blockIdx.y, blockIdx.z,
          // m_block, bidb, split_idx); }
