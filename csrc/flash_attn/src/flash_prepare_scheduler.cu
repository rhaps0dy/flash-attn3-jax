...
template <typename T> struct PrepareSortOp {
  __device__ __forceinline__ bool operator()(T const &lhs, T const &rhs) { return lhs > rhs; }
};

template <> struct PrepareSortOp<int2> {
  __device__ __forceinline__ bool operator()(int2 const &lhs, int2 const &rhs) const {
    return lhs.x > rhs.x;
  }
};

template <> struct PrepareSortOp<int4> {
...
__global__ void prepare_varlen_num_blocks_kernel(
    ...
    int *const tile_count_semaphore, int *const num_m_blocks_ptr, int *const num_splits_dynamic_ptr,
    int *const varlen_batch_idx_ptr,
    // int* const num_n_blocks_ptr,
    int *const num_nheads_in_l2_ptr, bool enable_pdl, bool is_causal, bool packgqa,
  ...
  auto get_num_n_blocks = [&](int batch_idx) {
    int leftpad_k =
        batch_idx < num_batch && leftpad_k_ptr != nullptr ? leftpad_k_ptr[batch_idx] : 0;
    int seqlen;
    if (seqused_k) {
  ...
  if (int(gridDim.x) > 1 || num_splits_static == 1) {
    // set num splits for all batches to 1 (note that user expects num_splits_static to mean upper
    // bound on splits) for batch size > 992, we expect GPU occupancy to not be an issue except in
    // degenerate cases (e.g., most are zero-length)
...
#pragma unroll
    ...
    // 10% margin
    int blocks_per_sm =
        static_cast<int>(ceilf(float(total_blocks) * 1.1f * float(num_head) / float(num_sm)));
    // blocks_per_sm = std::max(1, blocks_per_sm);  // 1 is the minimum number of blocks per SM
    num_splits_dynamic = std::max(
        std::min((num_n_blocks + blocks_per_sm - 1) / blocks_per_sm, num_splits_static), 1);
  ...
  if constexpr (Sort) {
    ...
    // if (threadIdx.x == 0) {
    //     printf("Unsorted: num_n_blocks - num_m_blocks = %d, num_m_blocks = %d, num_splits = %d,
    //     batch_idx = %d.\n",
    ...
    // if (threadIdx.x == 0) {
    //     printf("Sorted: num_n_blocks - num_m_blocks = %d, num_m_blocks = %d, num_splits = %d,
    //     batch_idx = %d.\n",
    //         batch_coords[0].x, batch_coords[0].y, batch_coords[0].z, batch_coords[0].w);
    // } __syncthreads();
    ...
    if (is_causal) {
      ...
      batch_coords[0].x =
          blockn_divmod.div(batch_coords[0].x + batch_coords[0].y * blockm_divmod.divisor);
    }
    ...
    if (batch_idx < num_batch && lane < kNumBatchPerWarp) {
      ...
      // printf("idx = %d, num_m_blocks = %d, num_n_blocks = %d, num_split_static = %d,
      // num_splits_dynamic = %d\n", bidb_start + lane, num_m_blocks_ptr[bidb_start + lane],
      // num_n_blocks, num_splits_static, num_splits_dynamic);
    }
  }
}
...
void prepare_varlen_num_blocks(Flash_fwd_params &params, cudaStream_t stream, bool packgqa,
  ...
  // printf("block size = %d, element size = %d, headdim = %d, headdim_v = %d, size 1 kblock =
  ...
  BOOL_SWITCH(params.varlen_sort_batches, Sort, [&] {
    NUM_WARP_SWITCH(num_warps, NumWarps, [&] {
      flash::prepare_varlen_num_blocks_kernel<NumWarps, Sort>
          <<<num_ctas /*grid*/, 32 * NumWarps /*block*/, 0, stream>>>(
              params.seqlen_q, params.seqlen_k, params.seqlen_knew, params.cu_seqlens_q,
              params.cu_seqlens_k, params.cu_seqlens_knew, params.seqused_q, params.seqused_k,
              params.leftpad_k, params.b, !packgqa ? params.h : params.h_k, qhead_per_khead,
              params.num_sm, params.num_splits, cutlass::FastDivmod(blockM),
              cutlass::FastDivmod(blockN), params.tile_count_semaphore, params.num_m_blocks_ptr,
              params.num_splits_dynamic_ptr, params.varlen_batch_idx_ptr,
              // params.num_n_blocks_ptr,
              params.num_nheads_in_l2_ptr, enable_pdl, params.is_causal, packgqa,
              max_kvblocks_in_l2);
    });
  });
}
