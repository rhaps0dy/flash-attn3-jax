...
inline bool should_pack_gqa(bool varlen_q, int seqlen_q, int qhead_per_khead, int blockM) {
  ...
  if (varlen_q)
    return true;
  // Heuristic: PackGQA is a bit slower but can help if seqlen_q is small or not near a multiple of
  // kBlockM
  ...
  float pack_gqa_efficiency =
      float(seqlen_q * qhead_per_khead) / float(round_up(seqlen_q * qhead_per_khead, blockM));
...
inline int num_splits_heuristic(int total_mblocks, int num_SMs, int num_n_blocks, int num_m_blocks,
                                int size_one_kv_head, bool is_causal_or_local, int max_splits) {
  ...
  // If num_n_blocks is too small, use 1 split. For example, we never split for hdim = 128 and
  // seqlen_k = 512.
  if (num_n_blocks <= 4) {
    return 1;
  }
