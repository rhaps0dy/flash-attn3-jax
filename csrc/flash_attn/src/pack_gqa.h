...
template <int kBlockM, int kHeadDim, int NumThreads, typename Element> struct PackGQAManager {
  ...
  // Was trying to have each WG loading Q to the rows in sQ that only that WG needs so that we only
  // need to sync within each WG, but didn't seem to be any faster. using GmemLayoutAtomWG =
  // Layout<Shape <Int<128 / kGmemThreadsPerRow>, Int<NumThreads / 128>, Int<kGmemThreadsPerRow> >,
  //     Stride<Int<kGmemThreadsPerRow>, _128, _1>>;
  // using GmemTiledCopyQCpAsyncWG = decltype(
  //     make_tiled_copy(GmemCopyAtomCpAsync{},
  //                     GmemLayoutAtomNew{},
  //                     Layout<Shape<_1, _1, Int<kGmemElemsPerLoad>>>{}));  // Val layout, 8 or 16
  //                     vals per load
  ...
  template <int NumThreadsPerRow = kGmemThreadsPerRow, typename Engine, typename Layout,
            typename TensorC>
  CUTLASS_DEVICE static auto compute_ptr(Tensor<Engine, Layout> &tensor, TensorC const &tRows,
...
#pragma unroll
  ...
  CUTLASS_DEVICE static void load_Q(TensormQ const &mQ, // ((qhead_per_khead, seqlen_q), headdim)
    ...
    // Tensor tQcQ_ = gmem_thr_copy_Q_cp_async.partition_S(cute::flat_divide(cQ, _64{}));       //
    // (ACPY,ACPY_M,ACPY_K) -> (blk_m,blk_k) Tensor tQsQ_ =
    // gmem_thr_copy_Q_cp_async.partition_D(cute::flat_divide(sQ, _64{}));       //
    // (ACPY,ACPY_M,ACPY_K) -> (blk_m,blk_k) Tensor tQcQ = group_modes<1, rank(tQcQ_) - 1>(tQcQ_);
...
#pragma unroll
    for (int m = 0; m < size<1>(tQsQ); ++m) {
      ...
      Element const *q_ptr = reinterpret_cast<Element const *>(
          __shfl_sync(0xffffffff, reinterpret_cast<uint64_t>(tPrQPtr(m / kGmemThreadsPerRow)),
...
#pragma unroll
      ...
      } // Don't need to fill in 0s for sQ since we're not gonna write the output to gmem for those
        // rows
    }
  };
  ...
  store_LSE(TensormLSE &mLSE,           // ((qhead_per_khead, seqlen_q))
    ...
    Tensor taccOcO_row =
        make_tensor(taccOcO.data(), flash::convert_layout_acc_rowcol(taccOcO.layout()))(_, _0{});
    ...
    Tensor tPrLSEPtr = compute_ptr<kMmaThreadsPerRow>(mLSE, taccOcO_row, qhead_per_khead_divmod,
                                                      thread_idx, m_block);
...
#pragma unroll
    for (int mi = 0; mi < size(tLSErLSE); ++mi) {
      ...
      float *ptr_LSE_cur = reinterpret_cast<float *>(
          __shfl_sync(0xffffffff, reinterpret_cast<uint64_t>(tPrLSEPtr[0]), mi % kMmaThreadsPerRow,
                      kMmaThreadsPerRow));
  ...
  CUTLASS_DEVICE static void store_O(TensormO &mO,         // ((qhead_per_khead, seqlen_o), headdim)
                                     TensorrO const &tOrO, // (kBlockM, kHeadDim) split across
                                                           // threads according to gmem_tiled_copy_O
                                     cutlass::FastDivmod const &qhead_per_khead_divmod,
                                     int const thread_idx, int const seqlen_o, int const m_block) {
...
#pragma unroll
    for (int m = 0; m < size<1>(tOrO); ++m) {
      ...
      Element *o_ptr = reinterpret_cast<Element *>(
          __shfl_sync(0xffffffff, reinterpret_cast<uint64_t>(tPrOPtr(m / kGmemThreadsPerRow)),
                      m % kGmemThreadsPerRow, kGmemThreadsPerRow));
...
#pragma unroll
  ...
  CUTLASS_DEVICE static void store_O_direct(
      TensormO &mO,         // ((qhead_per_khead, seqlen_o), headdim)
      TensorrO const &tOrO, // (kBlockM, kHeadDim) split across threads according to tiled_mma
      TiledMma tiled_mma, cutlass::FastDivmod const &qhead_per_khead_divmod, int const thread_idx,
      int const seqlen_o, int const m_block) {
    ...
    auto thread_mma = tiled_mma.get_thread_slice(thread_idx);
    Tensor taccOcO = thread_mma.partition_C(caccO); // (MMA,MMA_M,MMA_K)
    Tensor taccOcO_rowcol =
        make_tensor(taccOcO.data(), flash::convert_layout_acc_rowcol(taccOcO.layout()));
    ...
    // We split the work among threads loading the same row of O, then __shfl_sync the pointers.
    Tensor mO_0 = mO(_, _0{});
    Tensor tPrOPtr = compute_ptr<kMmaThreadsPerRow>(mO_0, taccOcO_row, qhead_per_khead_divmod,
                                                    thread_idx, m_block);
...
#pragma unroll
    for (int m = 0; m < size<1>(tOrO_copy); ++m) {
      ...
      Element *o_ptr = reinterpret_cast<Element *>(
          __shfl_sync(0xffffffff, reinterpret_cast<uint64_t>(tPrOPtr[0]), m % kMmaThreadsPerRow,
                      kMmaThreadsPerRow));
