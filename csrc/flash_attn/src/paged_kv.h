...
struct PagedKVManager {
  // If KV_Same_Iter=false, then we do load_page_table(0), load_K(0), load_page_table(1), load_K(1),
  // load_V(0), load_page_table(2), load_K(2), load_V(1), etc. So we need to compute the V pointers
  // for the previous iteration.

  // LoadsPerRow_LB is the lower bound on number of loads per row in the K direction. This is useful
  // for rotary where we want each thread to have at least 2 loads per row.
  ...
  // We want each "row" to have 64 elements (128 bytes, i.e. 1 cache line). E.g. if hdim=128, we
  // want each thread to have 4 loads in the M direction and 2 vectorized load in the K direction.
  ...
  // We assume threads loading the same row are in the same warp. This is for an optimization in
  // PagedKV where these threads share the same page table entry and share the work of computing
  // pointers to paged K and paged V.
  ...
  using GmemLayoutAtomKVCpAsync =
      Layout<Shape<Int<NumThreads / kGmemThreadsPerRow>, Int<kGmemThreadsPerRow>>,
  ...
  using GmemTiledCopyKVStore = decltype(make_tiled_copy(
      Copy_Atom<AutoVectorizingCopyWithAssumedAlignment<128>, Element>{}, GmemLayoutAtomKVCpAsync{},
      Layout<Shape<_1, Int<kGmemElemsPerLoad>>>{})); // Val layout, 8 or 16 vals per load
  ...
  using TensorPageTable = decltype(make_tensor(make_gmem_ptr(static_cast<int const *>(nullptr)),
                                               ShapePageTable{}, StridePageTable{})(int(0), _));
  using TensorKV = decltype(make_tensor(make_gmem_ptr(static_cast<Element *>(nullptr)), ShapeKV{},
                                        StrideKV{})(_, _, int(0), _));
  using GmemThrCopyKVCpAsync = decltype(GmemTiledCopyKVCpAsync{}.get_thread_slice(int(0)));
  using TensortKcK = decltype(GmemTiledCopyKVCpAsync{}.get_thread_slice(int(0)).partition_D(
      cute::make_identity_tensor(Shape<Int<kBlockN>, Int<kHeadDim>>{})));
  using TensortKpK = decltype(make_tensor<bool>(
      make_shape(size<1>(TensortKcK{}), size<2>(TensortKcK{})), Stride<_0, _1>{}));
  using TensortVcV = decltype(GmemTiledCopyKVCpAsync{}.get_thread_slice(int(0)).partition_D(
      cute::make_identity_tensor(Shape<Int<kBlockN>, Int<kHeadDimV>>{})));
  using TensortVpV = decltype(make_tensor<bool>(
      make_shape(size<1>(TensortVcV{}), size<2>(TensortVcV{})), Stride<_0, _1>{}));
  ...
  using TensorPageOffset =
      decltype(make_tensor<cute::tuple<int, int>>(Shape<Int<kPageEntryPerThread>>{}));
  ...
  CUTLASS_DEVICE
  PagedKVManager(int const *const ptr_page_table_, ShapePageTable const &shape_pagetable,
                 StridePageTable const &stride_pagetable, Element *const ptr_K,
                 ShapeKV const &shape_K, StrideKV const &stride_K, Element *const ptr_V,
                 int const headdim_v, StrideKV const &stride_V,
                 cutlass::FastDivmod const &page_size_divmod,
                 cutlass::FastDivmod const &blockN_per_page_size_divmod, int const bidb,
                 int const bidh, int const thread_idx, int const seqlen_k, int const leftpad_k,
                 int bidb_kv_idx)
      : page_size_divmod(page_size_divmod),
        blockN_per_page_size_divmod(blockN_per_page_size_divmod), thread_idx(thread_idx),
        seqlen_k(seqlen_k), leftpad_k(leftpad_k), ptr_page_table(ptr_page_table_),
        gmem_thr_copy_kv(gmem_tiled_copy_kv.get_thread_slice(thread_idx)), bidb_kv_idx(bidb_kv_idx),
        bidb_kv_idx_prev(bidb_kv_idx)

  {
    mPageTable =
        make_tensor(make_gmem_ptr(ptr_page_table), shape_pagetable, stride_pagetable)(bidb, _);
    ...
    tKpK = make_tensor<bool>(make_shape(size<1>(TensortKcK{}), size<2>(TensortKcK{})),
                             Stride<_0, _1>{});
...
#pragma unroll
    ...
    Tensor tVpV_ = make_tensor<bool>(make_shape(size<1>(TensortVcV{}), size<2>(TensortVcV{})),
                                     Stride<_0, _1>{});
    Tensor cV = cute::make_identity_tensor(
        Shape<Int<kBlockN>, Int<kHeadDimV>>{}); // (BLK_N,BLK_K) -> (blk_n,blk_k)
...
// The uncoalesced gmem load is intentional. This is so that each thread only loads the page table
// entries it needs, and we don't need any sync between warps. Assuming 8 threads per row, and 176
// rows, then the rows from 0 to 175 are loaded by threads 0, 8, 16, ..., 120, 1, 9, ..., 121, 2,
// 10, ..., 122, etc.
#pragma unroll
    for (int i = 0; i < kPageEntryPerThread; ++i) {
      ...
      // if (cute::thread0()) { printf("row = %d, page_idx = %d, page_offset = %d, page = %d,
      // leftpad_k = %d, seqlen_k = %d\n", row, page_idx, page_offset, page, leftpad_k, seqlen_k); }
    }
  ...
  template <bool First_iter = false> CUTLASS_DEVICE void load_page_table_TMA(const int n_block) {
  ...
  CUTLASS_DEVICE
  cute::tuple<int, int> get_indices_for_K_TMA() { return {n_block_idx, bidb_kv_idx}; };
...
#pragma unroll
  ...
  CUTLASS_DEVICE void load_K(const int n_block, TensorK &&sK) {
    ...
    // So we subtract the limit by the first row index of this thread (get<0>(tKcK(_0{}, _0{},
    // _0{})))
...
#pragma unroll
    for (int m = 0; m < size<1>(tKsK); ++m) {
      bool const should_load =
          EvenN ? (!Seqlenk_mask || get<0>(t0KcK(_0{}, m, _0{})) < seqlenk_row_limit)
                : get<0>(t0KcK(_0{}, m, _0{})) < seqlenk_row_limit;
      Element const *k_ptr = reinterpret_cast<Element const *>(
          __shfl_sync(0xffffffff, reinterpret_cast<uint64_t>(tPrKPtr(m / kGmemThreadsPerRow)),
...
#pragma unroll
        for (int k = 0; k < size<2>(tKsK); ++k) {
          ...
          cute::copy(gmem_tiled_copy_kv.with(tKpK(_0{}, k)), mK_paged_cur_copy(_, ki),
                     tKsK(_, m, k));
        }
...
#pragma unroll
    for (int m = 0; m < size<1>(tVsV); ++m) {
      ...
      if (EvenN || m < size<1>(tVsV) - 1 || get<0>(tVcV(_0{}, m, _0{})) < kBlockN) {
        ...
        Element const *v_ptr = reinterpret_cast<Element const *>(
            __shfl_sync(0xffffffff, reinterpret_cast<uint64_t>(tPrVPtr(m / kGmemThreadsPerRow)),
                        m % kGmemThreadsPerRow, kGmemThreadsPerRow));
...
#pragma unroll
  ...
  template <typename TensorK> CUTLASS_DEVICE void store_K(const int n_block, TensorK &&tKrK) {
    Tensor tPrKPtr = compute_K_ptr();
    ...
    // We want to use the row indices of thread0 to compare, since that is known at compile time.
    // So we subtract the limit by the first row index of this thread (get<0>(tKcK(_0{}, _0{},
    // _0{}))) int const seqlenk_row_limit = seqlen_k - n_block * kBlockN - get<0>(tKcK(_0{}, _0{},
    // _0{}));
    int const seqlenk_row_limit =
        std::min(seqlen_k - n_block * kBlockN, kBlockN) - get<0>(tKcK(_0{}, _0{}, _0{}));
// if (threadIdx.x == 128) { printf("bidx = %d, bidy = %d, bidz = %d, seqlen_k = %d,
...
#pragma unroll
    for (int m = 0; m < size<1>(tKrK); ++m) {
      ...
      Element *k_ptr = reinterpret_cast<Element *>(
          __shfl_sync(0xffffffff, reinterpret_cast<uint64_t>(tPrKPtr(m / kGmemThreadsPerRow)),
                      (m % kGmemThreadsPerRow), kGmemThreadsPerRow));
      Tensor mK_paged_cur = make_tensor(make_gmem_ptr(k_ptr), Shape<Int<kHeadDim>>{});
      Tensor mK_paged_cur_copy = cute::tiled_divide(mK_paged_cur, Shape<Int<kGmemElemsPerLoad>>{});
...
#pragma unroll
  ...
  template <typename TensorV> CUTLASS_DEVICE void store_V(const int n_block, TensorV &&tVrV) {
    if constexpr (KV_Same_Iter) {
      compute_V_ptr();
    }
    // Only for index calculation, since all the indices of thread 0 are known at compile time
    ...
    GmemTiledCopyKVStore gmem_tiled_copy_kv_store;
    int const seqlenk_row_limit =
        std::min(seqlen_k - n_block * kBlockN, kBlockN) - get<0>(tVcV(_0{}, _0{}, _0{}));
#pragma unroll
    for (int m = 0; m < size<1>(tVrV); ++m) {
      bool const should_load = get<0>(t0VcV(_0{}, m, _0{})) < seqlenk_row_limit;
      Element *v_ptr = reinterpret_cast<Element *>(
          __shfl_sync(0xffffffff, reinterpret_cast<uint64_t>(tPrVPtr(m / kGmemThreadsPerRow)),
                      m % kGmemThreadsPerRow, kGmemThreadsPerRow));
      Tensor mV_paged_cur = make_tensor(make_gmem_ptr(v_ptr), Shape<Int<kHeadDimV>>{});
