...
template <bool Varlen, int kBlock> struct SeqlenInfo {
  ...
  CUTLASS_DEVICE
  SeqlenInfo(int const bidb, int const seqlen_static, int const *const cu_seqlens,
             int const *const seqused)
      : offset(!Varlen || cu_seqlens == nullptr ? 0 : cu_seqlens[bidb]),
        offset_padded(!Varlen || cu_seqlens == nullptr
                          ? 0
                          : (cu_seqlens[bidb] + bidb * kBlock) / kBlock * kBlock),
        seqlen(!Varlen ? seqlen_static
                       : (seqused ? seqused[bidb]
                                  : (cu_seqlens ? cu_seqlens[bidb + 1] - cu_seqlens[bidb]
                                                : seqlen_static))) {}
};
...
template <bool Varlen, int kBlockM> struct SeqlenInfoQK {
  ...
  SeqlenInfoQK(int const bidb, int const seqlen_q_static, int const seqlen_k_static,
      ...
      : offset_q(!Varlen || cu_seqlens_q == nullptr ? 0 : cu_seqlens_q[bidb]),
        ...
        // If varlen, the layout for dPSum, LSE_log2, and dQaccum is that we pad each sequence in
        // the batch by an extra kBlockM, so that the write for each sequence doesn't touch the next
        // sequence. Sequence i starts at cu_seqlens[i] + i * kBlockM and ends at cu_seqlens[i + 1]
        // + i * kBlockM However, the start must align to multiples of kBlockM.
        ,
...
template <bool Varlen, bool AppendKV> struct SeqlenInfoQKNewK {
  ...
  CUTLASS_DEVICE
  SeqlenInfoQKNewK(int const bidb, int const seqlen_q_static, int const seqlen_k_static,
                   int const shape_K_new_0, int const *const cu_seqlens_q,
                   int const *const cu_seqlens_k, int const *const cu_seqlens_k_new,
                   int const *const seqused_q, int const *const seqused_k,
                   int const *const ptr_leftpad_k, int const *const seqlens_rotary)
      : leftpad_k(ptr_leftpad_k ? ptr_leftpad_k[bidb] : 0),
        ...
        seqlen_k_new(!AppendKV
                         ? 0
                         : (cu_seqlens_k_new ? cu_seqlens_k_new[bidb + 1] - cu_seqlens_k_new[bidb]
                                             : shape_K_new_0)),
