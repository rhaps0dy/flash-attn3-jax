...
template <bool Scale_max = true, bool Check_inf = true, int Max_offset = 0, typename Engine0,
...
#pragma unroll
  ...
  __forceinline__ __device__ void online_softmax(Tensor0 &acc_s) {
    ...
    Tensor scores = make_tensor(acc_s.data(), flash::convert_layout_acc_rowcol(acc_s.layout()));
    static_assert(CUTE_STATIC_V(size<0>(scores)) == kNRows);
    flash::template scale_apply_exp2</*Scale_max=*/true, Check_inf, Max_offset>(scores, row_max,
                                                                                softmax_scale_log2);
...
#pragma unroll
    for (int mi = 0; mi < size(row_sum); ++mi) {
      ...
      // For FP8, we might have scaled the output of exp by 2**8 so we need to divide sum by that
      // amount.
