...
tile_size_fwd_sm90(int headdim, int headdim_v, bool is_causal, bool is_local, int element_size = 2,
  ...
  if (element_size == 2) {
    if (headdim <= 64) {
      ...
      return {128, paged_kv_non_TMA || is_local ? 96 : (headdim_v <= 128 ? 128 : 112), true,
    ...
    if (headdim <= 64) {
      ...
      return {128, paged_kv_non_TMA ? 160 : (v_colmajor || (softcap && is_local) ? 192 : 224), true,
              true};
    } else if (headdim <= 192) {
      return {128, (paged_kv_non_TMA || softcap) && is_local ? 128 : 160, true, true};
    } else {
      return {128, is_local ? 64 : 128, true,
              !paged_kv_non_TMA}; // PagedKV uses more registers so we disabled IntraWGOverlap
    }
  }
}
...
tile_size_fwd_sm8x(bool sm86_or_89, int headdim, int headdim_v, bool is_causal, bool is_local,
  ...
  if (element_size == 2) {
    if (headdim <= 64) {
      ...
      return {128,
              use_8_warps ? (varlen_and_split ? (is_local ? 96 : 112) : (is_local ? 96 : 128))
                          : (is_local ? 48 : 64),
      ...
      return {128,
              sm86_or_89 ? (append_kv ? 32 : (varlen_and_split || is_local ? 48 : 64))
