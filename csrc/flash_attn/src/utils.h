...
// For SM90, FP16/BF16, convert acc_layout from ((2, 2, N / 8), MMA_M, MMA_N) to ((2, 2, 2), MMA_M,
// (N / 16, MMA_N)) For SM90, FP8, convert acc_layout from ((2, 2, N / 8), MMA_M, MMA_N) to ((4, 2,
// 2), MMA_M, (N / 32, MMA_N))
  ...
  if constexpr (decltype(rank<0>(acc_layout))::value == 3) { // SM90
    ...
    if constexpr (sizeof(typename MMA_Traits::ValTypeA) == 2) {
      ...
      return make_layout(
          ...
          coalesce(
              make_layout(get<0, 1>(l), get<2>(acc_layout)))); // ((4, 2, 2), MMA_M, N / 32 * MMA_N)
      // This combination is right but doesn't work with register shuffling.
      // return make_layout(make_layout(coalesce(make_layout(get<0, 0>(acc_layout), get<0, 0,
      // 0>(l))), get<0, 1>(acc_layout), get<0, 0, 1>(l)),
      //                    get<1>(acc_layout),
      //                    coalesce(make_layout(get<0, 1>(l), get<2>(acc_layout))));
    }
  } else { // SM80
...
CUTLASS_DEVICE auto convert_type_unsafe(Tensor<Engine, Layout> const &tensor) {
  ...
  auto frag =
      convert_op(*reinterpret_cast<const cutlass::Array<From_type, numel> *>(tensor.data()));
  return make_tensor(make_rmem_ptr<To_type>(&frag), tensor.layout());
  // Unsafe because we're returning a tensor with memory allocated on the stack. If the compiler
  // does not inline this function, then the memory might not be valid.
}
...
CUTLASS_DEVICE void convert_type_out(Tensor<Engine, Layout> const &tensor,
  ...
  // Somehow if we allocate out inside this function and return it, e2e is slower and the output can
  // be wrong.
  using From_type = typename Engine::value_type;
...
CUTLASS_DEVICE void gemm(TiledMma &tiled_mma, Tensor0 const &tCrA, Tensor1 const &tCrB,
                         Tensor2 &tCrC) {
  if constexpr (M_slice >= 0) {
    ...
    Tensor tCrC_slice = cute::logical_divide(tCrC, Shape<cute::Underscore, Int<MMA_M>>{})(
        _, make_coord(Int<M_slice>{}, _), _);
    if constexpr (!SwapAB) {
      Tensor tCrA_slice = cute::logical_divide(tCrA, Shape<cute::Underscore, Int<MMA_M>>{})(
          _, make_coord(Int<M_slice>{}, _), _);
      gemm<zero_init, wg_wait, SwapAB, /*M_slice=*/-1>(tiled_mma, tCrA_slice, tCrB, tCrC_slice);
    } else {
      Tensor tCrB_slice = cute::logical_divide(tCrB, Shape<cute::Underscore, Int<MMA_M>>{})(
          _, make_coord(Int<M_slice>{}, _), _);
    ...
    if constexpr (kNumKIters > kMaxKIters) {
      ...
      for (int k_block = kMaxKIters; k_block < kNumKIters; ++k_block) {
        if constexpr (!SwapAB) {
          cute::gemm(tiled_mma, tCrA(_, _, k_block + k_offset), tCrB(_, _, k_block + k_offset),
                     tCrC);
        } else {
          cute::gemm(tiled_mma, tCrB(_, _, k_block + k_offset), tCrA(_, _, k_block + k_offset),
                     tCrC);
        }
        tiled_mma.accumulate_ = GMMA::ScaleOut::One;
      }
...
template <bool A_in_regs = false, bool B_in_regs = false, bool SwapAB = false, typename Tensor0,
          typename Tensor1, typename Tensor2, typename Tensor3, typename Tensor4, typename TiledMma,
...
CUTLASS_DEVICE void gemm_sm80(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const &tCsA,
  ...
  if constexpr (SwapAB) {
    gemm_sm80<B_in_regs, A_in_regs>(acc, tCrB, tCrA, tCsB, tCsA, tiled_mma, smem_tiled_copy_B,
                                    smem_tiled_copy_A, smem_thr_copy_B, smem_thr_copy_A, fn);
  } else {
...
template <bool Is_even_MN = true, bool Is_even_K = true, bool Clear_OOB_MN = false,
          ...
          typename Layout0, typename Engine1, typename Layout1, typename Engine2, typename Layout2,
...
CUTLASS_DEVICE void copy(TiledCopy<CopyAtom, TV, Tiler> const &tiled_copy,
  ...
  auto has_with_bool = cute::is_valid(
      [](auto t) -> void_t<decltype(declval<typename decltype(t)::Traits>().with(true))> {},
      copy_atom);
#pragma unroll
...
#pragma unroll
        for (int k = 0; k < size<2>(S); ++k) {
          if constexpr (Is_even_K || !Clear_OOB_K) {
            ...
            // If copy traits can be transformed with a predicate value, do it, otherwise branch
            // here
...
#pragma unroll
        for (int k = 0; k < size<2>(S); ++k) {
          cute::copy(copy_atom.with(predicate_mn && (Is_even_K || predicate_K(k))), S(_, m, k),
                     D(_, m, k));
        }
      }
    }
...
template <typename Fragment> CUTLASS_DEVICE void permute_Aregs_fp8(Fragment &frag) {
...
template <typename Fragment> CUTLASS_DEVICE void permute_Cregs_fp8(Fragment &frag) {
...
template <typename Fragment> CUTLASS_DEVICE void permute_output_fp8(Fragment &out) {
...
template <typename Fragment> CUTLASS_DEVICE void permute_output_fp8_Vcolmajor(Fragment &frag) {
...
template <class T> CUTE_DEVICE T warp_uniform(T a) { return __shfl_sync(0xffffffff, a, 0); }

////////////////////////////////////////////////////////////////////////////////////////////////////

CUTLASS_DEVICE
int canonical_warp_group_idx_nosync() { return threadIdx.x / cutlass::NumThreadsPerWarpGroup; }
