...
inline std::ostream& operator<<(std::ostream& os, const XLA_FFI_ExecutionStage stage) {
  switch (stage) {
...
class Ffi {
 public:
  ...
  // given platform.
  static XLA_FFI_Error* RegisterStaticHandler(const XLA_FFI_Api* api, std::string_view name,
  ...
  // given platform.
  static XLA_FFI_Error* RegisterStaticHandler(const XLA_FFI_Api* api, std::string_view name,
                                              std::string_view platform, XLA_FFI_Handler* execute,
                                              XLA_FFI_Handler_Traits traits = 0) {
    return RegisterStaticHandler(
        api, name, platform, XLA_FFI_Handler_Bundle{nullptr, nullptr, nullptr, execute}, traits);
  }
  ...
  // `type_id`.
  static XLA_FFI_Error* RegisterTypeId(const XLA_FFI_Api* api, std::string_view name,
 ...
 protected:
  ...
  static XLA_FFI_Error* MakeError(const XLA_FFI_Api* api, XLA_FFI_Error_Code errc,
                                  std::string message);

  static XLA_FFI_Error* InvalidArgument(const XLA_FFI_Api* api, std::string message);

  static XLA_FFI_Error* CheckStructSize(const XLA_FFI_Api* api, std::string_view struct_name,
                                        size_t expected, size_t actual);

  static XLA_FFI_Error* StructSizeIsGreaterOrEqual(const XLA_FFI_Api* api,
                                                   std::string_view struct_name, size_t expected,
                                                   size_t actual);
};

inline XLA_FFI_Error* Ffi::RegisterStaticHandler(const XLA_FFI_Api* api, std::string_view name,
                                                 std::string_view platform,
                                                 XLA_FFI_Handler_Bundle bundle,
...
inline XLA_FFI_Error* Ffi::RegisterTypeId(const XLA_FFI_Api* api, std::string_view name,
                                          XLA_FFI_TypeId* type_id) {
...
inline XLA_FFI_Error* Ffi::MakeError(const XLA_FFI_Api* api, XLA_FFI_Error_Code errc,
                                     std::string message) {
...
inline XLA_FFI_Error* Ffi::InvalidArgument(const XLA_FFI_Api* api, std::string message) {
  return MakeError(api, XLA_FFI_Error_Code_INVALID_ARGUMENT, std::move(message));
}

inline XLA_FFI_Error* Ffi::CheckStructSize(const XLA_FFI_Api* api, std::string_view struct_name,
                                           size_t expected, size_t actual) {
  if (XLA_FFI_PREDICT_FALSE(expected != actual)) {
    return InvalidArgument(api, StrCat("Unexpected ", struct_name, " size: expected ", expected,
                                       " got ", actual, ". Check installed software versions."));
  }
  return nullptr;
}

inline XLA_FFI_Error* Ffi::StructSizeIsGreaterOrEqual(const XLA_FFI_Api* api,
                                                      std::string_view struct_name, size_t expected,
                                                      size_t actual) {
  if (XLA_FFI_PREDICT_FALSE(actual < expected)) {
    return InvalidArgument(
        api, StrCat("Unexpected ", struct_name, " size: expected at least ", expected, " got ",
...
template <typename... Ts>
using HasRemainingArgsTag = std::disjunction<std::is_same<RemainingArgsTag, Ts>...>;
...
template <typename... Ts>
using HasRemainingRetsTag = std::disjunction<std::is_same<RemainingRetsTag, Ts>...>;
...
constexpr XLA_FFI_DataType NativeTypeToCApiDataType() {
  if constexpr (std::is_same_v<T, char>) {
    ...
    static_assert(std::is_same_v<T, std::complex<double>>, "unsupported FFI data type");
...
class Binding {
 public:
  ...
  std::unique_ptr<Handler<stage, Fn, Ts...>> To(Fn fn, std::initializer_list<Traits> traits = {}) {
 ...
 private:
  ...
  explicit Binding() { static_assert(sizeof...(Ts) == 0, "arguments must be empty"); }
...
inline constexpr bool is_arg_binding_v = !std::is_void_v<typename ArgBinding<Param>::Arg>;

template <typename Param>
inline constexpr bool is_ret_binding_v = !std::is_void_v<typename RetBinding<Param>::Ret>;

template <typename Param>
inline constexpr bool is_attr_binding_v = !std::is_void_v<typename AttrBinding<Param>::Attr>;

template <typename Param>
inline constexpr bool is_attrs_binding_v = !std::is_void_v<typename AttrsBinding<Param>::Attrs>;

template <typename Param>
inline constexpr bool is_ctx_binding_v = !std::is_void_v<typename CtxBinding<Param>::Ctx>;
...
struct BindOne<Fn, Param, Params...> {
  ...
  static auto To(Fn fn, InFlightBinding binding, std::initializer_list<Traits> traits) {
    if constexpr (is_arg_binding_v<Param>) {
      // Bind parameter as an FFI handler argument.
      return BindOne<Fn, Params...>::To(
          std::move(fn), std::move(binding).template Arg<typename ArgBinding<Param>::Arg>(),
          traits);

    } else if constexpr (is_ret_binding_v<Param>) {
      // Bind parameter as an FFI handler result.
      return BindOne<Fn, Params...>::To(
          std::move(fn), std::move(binding).template Ret<typename RetBinding<Param>::Ret>(),
          traits);
      ...
      return BindOne<Fn, Params...>::To(
          std::move(fn), std::move(binding).template Attrs<typename AttrsBinding<Param>::Attrs>(),
          traits);

    } else if constexpr (is_ctx_binding_v<Param>) {
      // Bind parameter as an FFI handler context.
      return BindOne<Fn, Params...>::To(
          std::move(fn), std::move(binding).template Ctx<typename CtxBinding<Param>::Ctx>(),
          traits);

    } else {
      // Parameter is not recognized as one of the types that can be bound to
      // FFI handler.
      static_assert(sizeof(Param) == 0, "parameter is not supported for binding");
    }
  }
};
...
struct Bind<stage, ResultType (*)(Params...)> {
  ...
  static auto To(Fn fn, std::initializer_list<Traits> traits) {
    return BindOne<Fn, Params...>::To(std::move(fn), Ffi::Bind<stage>(), traits);
  }
};

// Binding specialization for callables (lambdas with captures).
template <ExecutionStage stage, typename ResultType, typename Fn, typename... Params>
...
auto Ffi::BindTo(Fn fn, std::initializer_list<Traits> traits) {
  if constexpr (std::is_pointer_v<Fn>) {
    ...
    return internal::Bind<stage, decltype(&Fn::operator())>::To(std::move(fn), traits);
  }
}
...
class InFlightDiagnostic {
 public:
  explicit InFlightDiagnostic(DiagnosticEngine* engine, std::string s) : engine_(engine) {
    stream_ << s;
  }
...
inline InFlightDiagnostic::~InFlightDiagnostic() { engine_->append(stream_.str()); }
...
struct Decode {
  ...
  static std::optional<T> call(DecodingOffsets& offsets, DecodingContext& ctx,
    ...
    return ArgDecoding<T>::Decode(ctx.call_frame->args.types[idx], ctx.call_frame->args.args[idx],
                                  diagnostic);
  }
};
...
struct Decode<OptionalArgTag<T>> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<std::optional<T>> call(DecodingOffsets& offsets, DecodingContext& ctx,
                                              DiagnosticEngine& diagnostic) {
...
struct Decode<RetTag<T>> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<Result<T>> call(DecodingOffsets& offsets, DecodingContext& ctx,
                                       DiagnosticEngine& diagnostic) {
    int64_t idx = offsets.rets++;
    return RetDecoding<T>::Decode(ctx.call_frame->rets.types[idx], ctx.call_frame->rets.rets[idx],
                                  diagnostic);
  }
};
...
struct Decode<OptionalRetTag<T>> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<std::optional<Result<T>>> call(DecodingOffsets& offsets,
                                                      DecodingContext& ctx,
                                                      DiagnosticEngine& diagnostic) {
  ...
  static std::optional<R> call(DecodingOffsets& offsets, DecodingContext& ctx,
    ...
    if (XLA_FFI_PREDICT_FALSE(!eq(attr_name, ctx.attrs_names[i]))) {
      return diagnostic.Emit("Attribute name mismatch: ")
             << std::string_view{attr_name->ptr, attr_name->len} << " vs " << ctx.attrs_names[i];
    }
...
struct Decode<CtxTag<T>> {
  ...
  static std::optional<R> call(DecodingOffsets& offsets, DecodingContext& ctx,
                               DiagnosticEngine& diagnostic) {
    return CtxDecoding<T>::Decode(ctx.call_frame->api, ctx.call_frame->ctx, diagnostic);
  }
};
...
class RemainingArgsBase {
 public:
  RemainingArgsBase(const XLA_FFI_Args* args, size_t offset) : args_(args), offset_(offset) {
...
class RemainingRetsBase {
 public:
  RemainingRetsBase(const XLA_FFI_Rets* rets, size_t offset) : rets_(rets), offset_(offset) {
...
class DictionaryBase {
 ...
 protected:
  ...
  template <typename T>
  std::optional<T> get(std::string_view name, DiagnosticEngine& diagnostic) const {
...
struct internal::Decode<internal::AttrsTag<T>> {
  static std::optional<T> call(DecodingOffsets& offsets, DecodingContext& ctx,
                               DiagnosticEngine& diagnostic) {
    return AttrDecoding<T>::Decode(XLA_FFI_AttrType_DICTIONARY, &ctx.call_frame->attrs, diagnostic);
  }
};
...
class Handler : public Ffi {
  ...
  static constexpr int64_t kNumRets = internal::NumTagged<internal::RetTag, Ts...>::value;

  static constexpr int64_t kNumOptionalRets =
      internal::NumTagged<internal::OptionalRetTag, Ts...>::value;

  static constexpr int64_t kNumAttrs = internal::NumTagged<internal::AttrTag, Ts...>::value;

  static constexpr int64_t kNumDictAttrs = internal::NumTagged<internal::AttrsTag, Ts...>::value;
 ...
 public:
  ...
  XLA_FFI_Error* Call(XLA_FFI_CallFrame* call_frame) const override {
    ...
    if (XLA_FFI_PREDICT_FALSE(call_frame->stage != static_cast<XLA_FFI_ExecutionStage>(stage))) {
      return InvalidArgument(call_frame->api, StrCat("Wrong execution stage: expected `",
    ...
    if constexpr (internal::HasRemainingArgsTag<Ts...>::value) {
      if (XLA_FFI_PREDICT_FALSE(call_frame->args.size < kNumArgs)) {
        return InvalidArgument(
            call_frame->api,
            StrCat("Wrong number of arguments: expected at least ", kNumArgs - kNumOptionalArgs - 1,
      ...
      if (XLA_FFI_PREDICT_FALSE(call_frame->args.size < kNumArgs)) {
        return InvalidArgument(
            call_frame->api,
            StrCat("Wrong number of arguments: expected at least ", kNumArgs - kNumOptionalArgs,
                   " but got ", call_frame->args.size));
      }
    } else {
      if (XLA_FFI_PREDICT_FALSE(call_frame->args.size != kNumArgs)) {
    ...
    if constexpr (internal::HasRemainingRetsTag<Ts...>::value) {
      if (XLA_FFI_PREDICT_FALSE(call_frame->rets.size < kNumRets)) {
        return InvalidArgument(
            call_frame->api,
            StrCat("Wrong number of results: expected at least ", kNumRets - kNumOptionalRets - 1,
      ...
      if (XLA_FFI_PREDICT_FALSE(call_frame->rets.size < kNumRets)) {
        return InvalidArgument(
            call_frame->api,
            StrCat("Wrong number of results: expected at least ", kNumRets - kNumOptionalRets,
                   " but got ", call_frame->rets.size));
      }
    } else {
      if (XLA_FFI_PREDICT_FALSE(call_frame->rets.size != kNumRets)) {
    ...
    if (XLA_FFI_PREDICT_FALSE(kNumDictAttrs == 0 && call_frame->attrs.size != kNumAttrs)) {
      ...
      if (call_frame->attrs.size > 0) {
        ...
        for (int64_t n = 0; n < call_frame->attrs.size - 1; ++n) {
          msg << std::string_view(call_frame->attrs.names[n]->ptr, call_frame->attrs.names[n]->len)
              << ", ";
        }
        msg << std::string_view(call_frame->attrs.names[call_frame->attrs.size - 1]->ptr,
 ...
 private:
  ...
  template <size_t... Is>
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE XLA_FFI_Error* Call(XLA_FFI_CallFrame* call_frame,
    ...
    internal::DecodingContext ctx = {call_frame, attrs_.data(), attrs_idx_.data()};
    ...
    if constexpr (sizeof...(Ts) > 0) {
      ...
      if (XLA_FFI_PREDICT_FALSE(!all_decoded)) {
        return FailedDecodeError(call_frame, {std::get<Is>(args).has_value()...}, diagnostic);
      }
    }

    ResultType result = fn_(std::move(*std::get<Is>(args))...);
    auto encoded = ResultEncoding<stage, ResultType>::Encode(call_frame->api, call_frame->ctx,
                                                             std::move(result));
    ...
    static constexpr bool kIsEncodedError = std::is_same_v<decltype(encoded), XLA_FFI_Error*>;
    static constexpr bool kIsEncodedFuture = std::is_same_v<decltype(encoded), XLA_FFI_Future*>;
    static constexpr bool kIsEncodedErrorOrFuture =
        std::is_same_v<decltype(encoded), std::variant<XLA_FFI_Error*, XLA_FFI_Future*>>;

    static_assert(kIsEncodedError || kIsEncodedFuture || kIsEncodedErrorOrFuture,
  ...
  Handler(Fn fn, std::vector<Traits> traits, std::vector<std::string> attrs)
      : fn_(std::move(fn)), traits_(std::move(traits)), attrs_(std::move(attrs)) {
    ...
    sorted.erase(std::unique(sorted.begin(), sorted.end(), std::equal_to<std::string>()),
                 sorted.end());

    // Find index of every attribute in the sorted attributes vector.
    for (size_t i = 0; i < attrs_.size(); ++i) {
      attrs_idx_.push_back(
          std::distance(sorted.begin(), std::find(sorted.begin(), sorted.end(), attrs_[i])));
    }
  }
...
#define XLA_FFI_REGISTER_SCALAR_ATTR_DECODING(T, TYPE)                                             \
  ...
  struct AttrDecoding<T> {                                                                         \
    using Type = T;                                                                                \
    XLA_FFI_ATTRIBUTE_ALWAYS_INLINE static std::optional<T> Decode(XLA_FFI_AttrType type,          \
                                                                   void* attr,                     \
                                                                   DiagnosticEngine& diagnostic) { \
...
XLA_FFI_REGISTER_SCALAR_ATTR_DECODING(std::complex<float>, XLA_FFI_DataType_C64);
XLA_FFI_REGISTER_SCALAR_ATTR_DECODING(std::complex<double>, XLA_FFI_DataType_C128);
...
struct DecodeDictionaryAttr {
  ...
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE static std::optional<T> Decode(
    ...
    std::tuple<std::optional<Ts>...> members = {dict.get<Ts>(names[Is], diagnostic)...};
...
#define XLA_FFI_REGISTER_STRUCT_ATTR_DECODING(T, ...)                                              \
  ...
  struct AttrDecoding<T> {                                                                         \
    ...
    XLA_FFI_ATTRIBUTE_ALWAYS_INLINE static std::optional<T> Decode(XLA_FFI_AttrType type,          \
      ...
      return decltype(decoder)::Decode(reinterpret_cast<const XLA_FFI_Attrs*>(attr),               \
      ...
      if (XLA_FFI_PREDICT_FALSE(attr_type != XLA_FFI_AttrType_SCALAR)) {                   \
        return diagnostic.Emit("Wrong attribute type: expected ")                          \
               << XLA_FFI_AttrType_SCALAR << " but got " << attr_type;                     \
                                                                                           ...
                                                                                           \
      auto* scalar = reinterpret_cast<XLA_FFI_Scalar*>(attr);                              \
      constexpr auto expected_dtype = ::xla::ffi::internal::NativeTypeToCApiDataType<U>(); \
...
#define XLA_FFI_DEFINE_HANDLER(fn, impl, ...)                                                  \
  XLA_FFI_DEFINE_HANDLER_X(, fn, impl, ##__VA_ARGS__,                                          \
...
#define XLA_FFI_REGISTER_HANDLER(API, NAME, PLATFORM, FUNC, ...) \
  XLA_FFI_REGISTER_HANDLER_(API, NAME, PLATFORM, FUNC, __COUNTER__, ##__VA_ARGS__)
...
#define XLA_FFI_REGISTER_HANDLER__(API, NAME, PLATFORM, FUNC, N, ...)                             \
  XLA_FFI_ATTRIBUTE_UNUSED static const XLA_FFI_Error* xla_ffi_static_handler_##N##_registered_ = \
      [] {                                                                                        \
        return ::xla::ffi::Ffi::RegisterStaticHandler(API, NAME, PLATFORM, FUNC, ##__VA_ARGS__);  \
      }()
...
#define XLA_FFI_DEFINE_HANDLER_SYMBOL(fn, impl, ...)                                     \
  extern "C" XLA_FFI_Error* fn(XLA_FFI_CallFrame* call_frame) {                          \
    ...
    return (*handler)(call_frame);                                                       \
                                                                                         ...
                                                                                         \
  static_assert(std::is_invocable_r_v<XLA_FFI_Error*, decltype(fn), XLA_FFI_CallFrame*>, \
