...
class Error {
 public:
  ...
  Error(ErrorCode errc, std::string message) : errc_(errc), message_(std::move(message)) {}
...
template <typename T, typename E>
constexpr Expected<T, E>::Expected(Unexpected<E> u) : data_(std::move(u.error_)) {}
...
class Future {
 ...
 private:
  ...
  struct WaiterAndState {
    ...
    Waiter* waiter() const { return reinterpret_cast<Waiter*>(value & kPointerMask); }
...
class CountDownPromise {
 public:
  ...
  explicit CountDownPromise(int64_t count) : CountDownPromise(Promise(), count) {}
  ...
  bool CountDown(size_t count, const Error& error = Error::Success()) {
    ...
    bool is_complete = state_->count.fetch_sub(count, std::memory_order_acq_rel) == count;
...
inline Future::Future(const Promise& promise) : data_(promise.data_) {
  assert(data_.use_count() == 2 && "Promise can be used to create at most one Future");
}

inline Future::Future(const CountDownPromise& promise) : Future(promise.AsPromise()) {}
...
void Future::OnReady(F&& f) {
  ...
  WaiterAndState old_value = data_->waiter_and_state.load(std::memory_order_acquire);
  ...
  while (!data_->waiter_and_state.compare_exchange_weak(
      old_value, new_value, std::memory_order_acq_rel, std::memory_order_acquire)) {
...
inline void Promise::SetCompleted(Future::State state) {
  Future::WaiterAndState old_value =
      data_->waiter_and_state.exchange({nullptr, state}, std::memory_order_acq_rel);
...
class AnyBuffer {
 public:
  ...
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE size_t element_count() const {
    ...
    return std::accumulate(dims.begin(), dims.end(), int64_t{1}, std::multiplies<>());
  }
...
constexpr bool IsComplexType() {
  return std::is_same_v<NativeType<dtype>, std::complex<NativeType<ToReal(dtype)>>>;
}
...
class Buffer {
 public:
  ...
  Dimensions dimensions() const {
    return Dimensions(buf_->dims, rank == internal::kDynamicRank ? buf_->rank : rank);
  }
...
XLA_FFI_ATTRIBUTE_ALWAYS_INLINE std::optional<Buffer<dtype, rank>> DecodeBuffer(
  ...
  if (auto buf_dtype = static_cast<DataType>(buf->dtype);
    ...
    return diagnostic.Emit("Wrong buffer dtype: expected ") << dtype << " but got " << buf_dtype;
  }

  if constexpr (rank != internal::kDynamicRank) {
    if (XLA_FFI_PREDICT_FALSE(buf->rank != rank)) {
      return diagnostic.Emit("Wrong buffer rank: expected ") << rank << " but got " << buf->rank;
    }
  }
...
struct ArgDecoding<Buffer<dtype, rank>> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<Buffer<dtype, rank>> Decode(XLA_FFI_ArgType type, void* arg,
                                                   DiagnosticEngine& diagnostic) {
    if (XLA_FFI_PREDICT_FALSE(type != XLA_FFI_ArgType_BUFFER)) {
      return diagnostic.Emit("Wrong argument type: expected ")
             << XLA_FFI_ArgType_BUFFER << " but got " << type;
    ...
    return internal::DecodeBuffer<dtype, rank>(reinterpret_cast<XLA_FFI_Buffer*>(arg), diagnostic);
  }
};
 ...
 public:
  ...
  ErrorOr<T> get(size_t index) const {
    ...
    if (XLA_FFI_PREDICT_FALSE(idx >= args()->size)) {
      return Unexpected(Error(ErrorCode::kInvalidArgument, "Index out of range"));
    }

    DiagnosticEngine diagnostic;
    std::optional<T> value =
        ArgDecoding<T>::Decode(args()->types[idx], args()->args[idx], diagnostic);
...
struct internal::Decode<internal::RemainingArgsTag> {
  static std::optional<RemainingArgs> call(DecodingOffsets& offsets, DecodingContext& ctx,
                                           DiagnosticEngine& diagnostic) {
...
struct RetDecoding<AnyBuffer> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<Result<AnyBuffer>> Decode(XLA_FFI_RetType type, void* ret,
                                                 DiagnosticEngine& diagnostic) {
...
struct RetDecoding<Buffer<dtype, rank>> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<Result<Buffer<dtype, rank>>> Decode(XLA_FFI_RetType type, void* ret,
                                                           DiagnosticEngine& diagnostic) {
    if (XLA_FFI_PREDICT_FALSE(type != XLA_FFI_RetType_BUFFER)) {
      return diagnostic.Emit("Wrong result type: expected ")
             << XLA_FFI_RetType_BUFFER << " but got " << type;
    ...
    return internal::DecodeBuffer<dtype, rank>(reinterpret_cast<XLA_FFI_Buffer*>(ret), diagnostic);
  }
};
 ...
 public:
  ...
  ErrorOr<Result<T>> get(size_t index) const {
    ...
    std::optional<Result<T>> value =
        RetDecoding<T>::Decode(rets()->types[idx], rets()->rets[idx], diagnostic);
    if (XLA_FFI_PREDICT_FALSE(!value.has_value())) {
      return Unexpected(Error::Internal(diagnostic.Result()));
    }
...
struct internal::Decode<internal::RemainingRetsTag> {
  static std::optional<RemainingRets> call(DecodingOffsets& offsets, DecodingContext& ctx,
                                           DiagnosticEngine& diagnostic) {
...
struct AttrDecoding<Pointer<T>> {
  ...
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE static std::optional<Type> Decode(XLA_FFI_AttrType type,
                                                                    void* attr,
                                                                    DiagnosticEngine& diagnostic) {
...
struct internal::Decode<internal::AttrsTag<Dictionary>> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE static std::optional<Dictionary> call(
      DecodingOffsets& offsets, DecodingContext& ctx, DiagnosticEngine& diagnostic) {
...
inline const char* GetErrorMessage(const XLA_FFI_Api* api, XLA_FFI_Error* error) {
...
struct ResultEncoding<stage, Error> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static XLA_FFI_Error* Encode(const XLA_FFI_Api* api, XLA_FFI_ExecutionContext* ctx, Error error) {
...
template <typename T>
struct ResultEncoding<ExecutionStage::kInstantiate, ErrorOr<std::unique_ptr<T>>> {
  static_assert(std::is_same_v<decltype(T::id), TypeId>,
                "State type must have a static `TypeId id` field");

  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static XLA_FFI_Error* Encode(const XLA_FFI_Api* api, XLA_FFI_ExecutionContext* ctx,
...
struct ResultEncoding<stage, Future> {
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::variant<XLA_FFI_Error*, XLA_FFI_Future*> Encode(const XLA_FFI_Api* api,
...
struct CtxDecoding<PlatformStream<T>> {
  ...
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<Type> Decode(const XLA_FFI_Api* api, XLA_FFI_ExecutionContext* ctx,
                                    DiagnosticEngine& diagnostic) {
    ...
    if (XLA_FFI_Error* error = api->XLA_FFI_Stream_Get(&args)) {
      diagnostic.Emit("Failed to get platform stream: ") << internal::GetErrorMessage(api, error);
...
struct CtxDecoding<ScratchAllocator> {
  ...
  XLA_FFI_ATTRIBUTE_ALWAYS_INLINE static std::optional<Type> Decode(const XLA_FFI_Api* api,
                                                                    XLA_FFI_ExecutionContext* ctx,
                                                                    DiagnosticEngine& diagnostic) {
...
inline std::optional<void*> ScratchAllocator::Allocate(size_t size, size_t alignment) {
...
inline ScratchAllocator::ScratchAllocator(const XLA_FFI_Api* api, XLA_FFI_ExecutionContext* ctx,
                                          DiagnosticEngine& diagnostic)
...
class ThreadPool {
 ...
 private:
  ...
  ThreadPool(const XLA_FFI_Api* api, XLA_FFI_ExecutionContext* ctx, DiagnosticEngine& diagnostic);
...
inline ThreadPool::ThreadPool(const XLA_FFI_Api* api, XLA_FFI_ExecutionContext* ctx,
                              DiagnosticEngine& diagnostic)
    : api_(api), ctx_(ctx), diagnostic_(diagnostic) {}
...
#define XLA_FFI_REGISTER_TYPE_(API, NAME, TYPE_ID, N) XLA_FFI_REGISTER_TYPE__(API, NAME, TYPE_ID, N)
#define XLA_FFI_REGISTER_TYPE__(API, NAME, TYPE_ID, N)                                       \
  XLA_FFI_ATTRIBUTE_UNUSED static const XLA_FFI_Error* xla_ffi_type_##N##_registered_ = [] { \
