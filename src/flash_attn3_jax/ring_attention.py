...
def ring_fwd(
    q, k, v, axis_name, axis_size, mha_fwd, softmax_scale=None, is_causal=False
):
    [n, l, h, d] = q.shape
    ...
    def f(c, a):
        ...
        if is_causal:
            ...
            o2, lse2 = jax.lax.switch(
                ...
                [
                    lambda: (
                        jnp.zeros([n, l, h, d], q.dtype),
                        jnp.full([n, h, l], float("-inf"), jnp.float32),
                    ),
                    lambda: mha_fwd(
                        q,
                        k,
        ...
        o = o1 * rearrange(jnp.exp(lse1 - lse), "n h l -> n l h 1") + o2 * rearrange(
            jnp.exp(lse2 - lse), "n h l -> n l h 1"
        )

        k2 = jax.lax.ppermute(
            k, axis_name, [(i, (i + 1) % axis_size) for i in range(axis_size)]
        )
        v2 = jax.lax.ppermute(
            v, axis_name, [(i, (i + 1) % axis_size) for i in range(axis_size)]
        )
        k_ix = jax.lax.ppermute(
            k_ix, axis_name, [(i, (i + 1) % axis_size) for i in range(axis_size)]
        )
...
def ring_bwd(
    ...
    def f(acc, _):
        ...
        (dk2_, dv2_) = jax.lax.ppermute(
            (dk2, dv2), axis_name, [(i, (i + 1) % axis_size) for i in range(axis_size)]
        )
        (k2_, v2_, ix2_) = jax.lax.ppermute(
            (k2, v2, ix2),
            axis_name,
    ...
    (dk, dv) = jax.lax.ppermute(
        (dk, dv), axis_name, [(i, (i + 1) % axis_size) for i in range(axis_size)]
    )
