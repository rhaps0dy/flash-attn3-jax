...
def test_flash_bwd(n, seqlen, h, d, m, causal, local, dtype):
    ...
    def flash(qkv):
        return (
            flash_mha(*qkv, is_causal=bool(causal), window_size=window_size).sum() * A
        )
...
def test_flash_fwd_vmap(n, seqlen, h, d, causal, local, dtype):
    ...
    x = 4
    q = jax.random.normal(
        jax.random.PRNGKey(0), [x, n, seqlen, h, d], dtype=jnp.float32
    )
    k = jax.random.normal(
        jax.random.PRNGKey(1), [x, n, seqlen, h, d], dtype=jnp.float32
    )
    v = jax.random.normal(
        jax.random.PRNGKey(2), [x, n, seqlen, h, d], dtype=jnp.float32
    )
...
def test_flash_bwd_vmap(n, seqlen, h, d, causal, local, dtype):
    ...
    do = jax.random.normal(
        jax.random.PRNGKey(3), [x, n, seqlen, h, d], dtype=jnp.float32
    )
