...
def test_ring_fwd(seqlen, h, d, m, causal):
    ...
    with Mesh(np.array(devices), axis_names=("x",)) as mesh:
        ...
        k = jax.random.normal(
            jax.random.PRNGKey(1), [n, seqlen, h, d], dtype=jnp.float32
        )
        v = jax.random.normal(
            jax.random.PRNGKey(2), [n, seqlen, h, d], dtype=jnp.float32
        )
        o_ref = ref(q, k, v)
        o_ring = ring(q, k, v)
...
def test_ring_bwd(seqlen, h, d, m, causal):
    ...
    with Mesh(np.array(devices), axis_names=("x",)) as mesh:
        ...
        @partial(
            shard_map,
            mesh=mesh,
            in_specs=(P(None, "x", None, None),) * 4,
            out_specs=(P(None, "x", None, None),) * 3,
            check_rep=False,
        )
        def ring(q, k, v, do):
            softmax_scale = 1 / np.sqrt(q.shape[-1])
        ...
        q = jax.random.normal(
            jax.random.PRNGKey(0), [1, seqlen, h * m, d], dtype=jnp.float32
        )
        k = jax.random.normal(
            jax.random.PRNGKey(1), [1, seqlen, h, d], dtype=jnp.float32
        )
        v = jax.random.normal(
            jax.random.PRNGKey(2), [1, seqlen, h, d], dtype=jnp.float32
        )
        do = (
            jax.random.normal(
                jax.random.PRNGKey(3), [1, seqlen, h * m, d], dtype=jnp.float32
            )
            * A
        )
        o_ref = ref(q, k, v, do)
