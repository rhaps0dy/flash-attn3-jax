    ...
    if not local:
        with Mesh(np.array(devices), axis_names=("x",)) as mesh:
            ...
            assert adjacent_pairs == 0, (
                f"Found non-overlapped rotations: {adjacent_pairs} (expected 0). Decoded: {decoded}"
            )
            # (N-1) overlapped rotations to see all N blocks.
            assert overlapped_pairs == n - 1, (
                f"Wrong number of overlapped rotations: {overlapped_pairs} (expected exactly {n - 1}). Decoded: {decoded}"
            )
...
def test_flash_bwd_sharded_hlo(seqlen, h, d, m, causal, local, dtype):
    ...
    def flash(qkv):
        return (
            flash_mha(*qkv, is_causal=bool(causal), window_size=window_size) ** 2
        ).sum()
    ...
    if not local:
        with Mesh(np.array(devices), axis_names=("x",)) as mesh:
            ...
            assert overlapped_pairs == n, (
                f"Wrong number of overlapped rotations: {overlapped_pairs} (expected exactly {n}). Decoded: {decoded}"
            )
            assert adjacent_pairs == 1, (
                f"Wrong number of non-overlapped rotations: {adjacent_pairs} (expected exactly 1). Decoded: {decoded}"
            )
...
def test_flash_fwd_sharded(seqlen, h, d, m, causal, local, dtype):
    ...
    q = jax.random.normal(
        jax.random.PRNGKey(0), [n, seqlen, h * m, d], dtype=jnp.float32
    )
...
def test_flash_bwd_sharded(seqlen, h, d, m, causal, local, dtype):
    ...
    do = (
        jax.random.normal(
            jax.random.PRNGKey(3), [n, seqlen, h * m, d], dtype=jnp.float32
        )
        * A
    )
