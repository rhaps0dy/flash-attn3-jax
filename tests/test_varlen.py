...
def test_varlen_flash_fwd(m, h, d, causal, local, dtype, seqused_k_limit):
    ...
    q = jax.random.normal(
        jax.random.PRNGKey(0), [total_seqlen, h * m, d], dtype=jnp.float32
    )
    k = jax.random.normal(
        jax.random.PRNGKey(1), [total_seqlen, h, d], dtype=jnp.float32
    )
    v = jax.random.normal(
        jax.random.PRNGKey(2), [total_seqlen, h, d], dtype=jnp.float32
    )
    ...
    def ref(q, k, v):
        ...
        for i in range(b):
            ...
            out = out.at[None, fenceposts[i] : fenceposts[i + 1]].set(
                ref_mha(bq, bk, bv, is_causal=bool(causal), window_size=window_size)
            )
        return out
    ...
    out = flash_mha_varlen(
        ...
        k,
        v,
        cu_seqlens_q=fenceposts,
        cu_seqlens_k=fenceposts,
        seqused_k=seqused_k,
...
if __name__ == "__main__":
    print(
        flash_mha_varlen(
            ...
            jnp.zeros([4, 1, 64], dtype=jnp.float16),
            jnp.zeros([4, 1, 64], dtype=jnp.float16),
            cu_seqlens_q=jnp.array([0, 2, 4]),
            cu_seqlens_k=jnp.array([0, 2, 4]),
            seqused_k=None,
            max_seqlen_q=4,
            max_seqlen_k=4,
            softmax_scale=0.5,
